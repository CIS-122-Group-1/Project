    ...
    public static class GpaCalculator {
        ...
        public static void mainMenu() throws IllegalArgumentException, InterruptedException           // main menu method to drive the program until input to quit the program is entered
        {
            ...
            do {                                                // do/while block to drive the entire program endlessly until user wants to quit
                ...
                {
                    System.out.println("Invalid menu choice, try again.");                      // catch block for the above error so the program can continue running after letting the user know their error
                }
        ...
        public static void GPACalculator() throws InterruptedException          // method that calculates the gpa based off of some variables the user inputs.
        {
            ...
            System.out.print("Enter the number of classes taken: ");                        // prompting user for how many classes they wish to enter grades for, for desired semester
            int numClasses = scanner.nextInt();

            float calculatedHourCredit = 0;
            float overallCreditEarned = 0;
            float calculatedGPA;
            double hourCredits = 0;
            for (int i = 0; i < numClasses; i++) {                                                      // for loop to continue the following code for each class the user has taken
                ...
                try
                {
                    ...
                    String inputGrade = scanner.next();                                                             // storing letter grade from user input for class num i+1

                    System.out.print("Please enter the number of credits the course gives (1-4): ");
                    double inputCredit = scanner.nextDouble();                                                          // storing credits that can be earned from that class
                    overallCreditEarned += inputCredit;                                                 // calculating how many credits the user took in given semester
                    if (inputCredit > 4 || inputCredit < 1)                                                       // checking user input if the inputted credits is valid based off of standard credits
                    {
                        throw new InvalidAlgorithmParameterException();                                 // if not, throw an error that will terminate the program and state why
                    }

                    if (inputGrade.equalsIgnoreCase("a"))                                       // if block to check the user's input letter grade and to add the appropriate credit/hours for the letter grade.
                        hourCredits = inputCredit * 4.00;
                    else if (inputGrade.equalsIgnoreCase("a-"))
                        hourCredits = inputCredit * 3.67;
                    else if (inputGrade.equalsIgnoreCase("b+"))
                        hourCredits = inputCredit * 3.33;
                    else if (inputGrade.equalsIgnoreCase("b"))
                        hourCredits = inputCredit * 3.00;
                    else if (inputGrade.equalsIgnoreCase("b-"))
                        hourCredits = inputCredit * 2.67;
                    else if (inputGrade.equalsIgnoreCase("c+"))
                        hourCredits = inputCredit * 2.33;
                    else if (inputGrade.equalsIgnoreCase("c"))
                        hourCredits = inputCredit * 2.00;
                    else if (inputGrade.equalsIgnoreCase("c-"))
                        hourCredits = inputCredit * 1.67;
                    else if (inputGrade.equalsIgnoreCase("d+"))
                        hourCredits = inputCredit * 1.30;
                    else if (inputGrade.equalsIgnoreCase("d"))
                        hourCredits = inputCredit * 1.00;
                    else if (inputGrade.equalsIgnoreCase("f"))
                        hourCredits = inputCredit * 0.00;
                    else
                    {
                    ...
                    calculatedHourCredit += hourCredits;                                         // adding all credit/hours earned that were inputted for the given semester
                }
            ...
            calculatedGPA = calculatedHourCredit / overallCreditEarned;                                      // calculates overall gpa for the given semester
            System.out.println("GPA for semester "+ semester + ": " + calculatedGPA);     //shows the user the overall gpa they achieved for the given semester
            System.out.println("\n");
            String semesterGPA = semester + ": " + calculatedGPA;         // stores users gpa/semester string into a variable
            Thread.sleep(1000);
            ...
            String gpa = String.valueOf(calculatedGPA);
...
//        }
        ...
        public static void overallGPA()                                             // method to show the user the overall gpa they have from the stored semesters.
        {
            ...
            if (StoredSemesters.size() > 0)
            {
                ...
                double calculatedGPA = sumOfCredits / numGPAS;                           // expression that calculates the overall gpa from all semesters / number of semesters


                System.out.println("Total GPA from all semesters stored: " + calculatedGPA);
                System.out.println();
            }
            else
            {
